...
use crate::{Cursor, CursorManager, QueuePublisher, config};
...
impl<'a> StartUpCatchUpWorker<'a> {
    ...
    pub async fn dispatch(&self) {
        ...
        for (program_id, cursor) in cursors {
            let p_id = Pubkey::from_str(&program_id).unwrap(); // TODO rm unwrap
            match cursor {
                // cursor exists — fetch everything since the last known signature
                Cursor::Existing(model) => {
                    let until = model.last_signature.parse().unwrap(); // TODO rm unwrap
    ...
    async fn fetch_signatures(&self, program_id: &Pubkey, until: Option<Signature>) {
        ...
        loop {
            ...
            before = Some(sigs.last().unwrap().signature.parse().unwrap()); // TODO rm unwrap

            // Push those into Redis task queue
            for sig in sigs.into_iter().rev() {
                self.queue_publisher
                    .publish(program_id, sig.signature.parse().unwrap()) // TODO rm unwrap
                    .await;
            }
        }
    }

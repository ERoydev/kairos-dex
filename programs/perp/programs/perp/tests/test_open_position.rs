...
fn test_open_position_distributes_fees() {
    ...
    // Trader's own transfers only cover collateral + protocol fee: transfer_margin_to_vault
    // sends (margin - fee) and transfer_protocol_fee sends protocol_fees directly. The LP's
    // cut is drawn out of the market vault afterwards (credit_lp_pool), not from the trader,
    // so the trader's total outflow is margin - lp_fee, not the full margin.
    assert_eq!(
        trader_ata_before - token_balance(&env.svm, &env.trader_usdc_ata),
        margin - expected_lp_fee
    );
    assert_eq!(
        token_balance(&env.svm, &env.market_vault) - market_vault_before,
        expected_collateral - expected_lp_fee
    );
    assert_eq!(
